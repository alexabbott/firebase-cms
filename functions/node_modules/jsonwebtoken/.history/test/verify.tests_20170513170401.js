  ...
  describe.only('expiration', function () {
    ...
    describe('option: maxAge', function () {
      it('should error for claims issued before a certain timespan', function (done) {
        ...
        var options = {algorithms: ['HS256'], maxAge: '3s'};
      ...
      it('should not error for claims issued before a certain timespan but still inside clockTolerance timespan', function (done) {
        clock = sinon.useFakeTimers(1437018587500); // iat + 5.5s, exp - 4.5s
        var options = {algorithms: ['HS256'], maxAge: '5s', clockTolerance: 1 };
      ...
      it('can be more restrictive than expiration', function (done) {
        ...
        var options = {algorithms: ['HS256'], maxAge: '8s'};
      ...
      it('should error if maxAge is specified but there is no iat claim', function (done) {
        clock = sinon.useFakeTimers(1437018591400); // iat + 9.4s, exp - 0.6s
        var options = {algorithms: ['HS256'], maxAge: '10s'};
      ...
      it('should error using numeric maxAge when max age was exceeded', function(done) {
        // { foo: 'bar', iat: 1437018582, exp: 1437018682 }
        // time passed: 15s = 1437018597000
        var token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJmb28iOiJiYXIiLCJpYXQiOjE0MzcwMTg1ODIsImV4cCI6MTQzNzAxODY4Mn0.sQ5FD4FuW1_oQq0Wj0EB0QL1JnZqsMFHV9TUqezSiGg';
        clock = sinon.useFakeTimers(1437018597000);
        var options = { algorithms: ['HS256'], maxAge: 10 };
      ...
      it('should pass validation using numeric maxAge when max age was not exceeded', function(done) {
        // { foo: 'bar', iat: 1437018582, exp: 1437018682 }
        // time passed: 15s = 1437018597000
        var token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJmb28iOiJiYXIiLCJpYXQiOjE0MzcwMTg1ODIsImV4cCI6MTQzNzAxODY4Mn0.sQ5FD4FuW1_oQq0Wj0EB0QL1JnZqsMFHV9TUqezSiGg';
        clock = sinon.useFakeTimers(1437018597000);
        var options = { algorithms: ['HS256'], maxAge: 20 };
